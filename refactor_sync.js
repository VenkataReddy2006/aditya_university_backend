...
const fs = require("fs");
let content = fs.readFileSync("backend/src/shared/sync/sync.service.js", "utf8");

const syncWithCookieFn = `
...
async function syncAllWithCookie(username, cookieString) {
    const loginRes = ausService.loginWithCookie(cookieString);
    if (!loginRes.success) return { success: false, message: "Invalid cookie" };
    const cookieClient = loginRes.client;
...
content += syncWithCookieFn;

content = content.replace(/module.exports = \{/, "module.exports = {\n    syncAllWithCookie,");

fs.writeFileSync("backend/src/shared/sync/sync.service.js", content);
console.log("Updated sync.service.js successfully!");
